...
function renderHome(){const home=$('home');if(!home)return;let card=$('fuelDeficitCard');if(!card){card=document.createElement('div');card.id='fuelDeficitCard';card.className='card';const first=home.querySelector('.card');if(first?.nextSibling)home.insertBefore(card,first.nextSibling);else home.appendChild(card)}const s=settings(),eaten=calories(),planned=s.tdee-s.cal,current=s.tdee-eaten;const label=current>=0?'estimated deficit vs maintenance':'estimated surplus vs maintenance';card.innerHTML=`<div class="sectiontitle"><h2>Calorie deficit</h2><span class="pill">maintenance ${Math.round(s.tdee)}</span></div><div class="grid"><div class="metric"><b>${Math.round(s.tdee)}</b><small>estimated maintenance</small></div><div class="metric"><b>${Math.round(s.cal)}</b><small>daily calorie target</small></div><div class="metric"><b>${Math.round(eaten)}</b><small>eaten today</small></div><div class="metric"><b>${Math.abs(Math.round(current))}</b><small>${label}</small></div></div><p class="note">Planned deficit at your calorie target: about ${Math.max(0,Math.round(planned))} calories/day. This is an estimate and can be adjusted as your weight trend gives us better real-world data.</p>`}
function installSettings(){const settingsPage=$('settings');if(!settingsPage)return;let card=$('fuelMaintenanceSettings');if(!card){card=document.createElement('div');card.id='fuelMaintenanceSettings';card.className='card';const reset=$('resetAll')?.closest('.card');card.innerHTML='<div class="sectiontitle"><h2>Maintenance calories</h2></div><label>Estimated maintenance / TDEE<input id="sTdee" type="number" min="1000" max="6000" step="25"></label><div class="spacer"></div><button id="saveTdee" class="primary" style="width:100%">Save maintenance</button><p class="note">Fuel uses this number to show your estimated daily deficit and gives it directly to Fuel Coach. We can refine it later from your real intake and weight trend.</p>';settingsPage.insertBefore(card,reset||null);$('saveTdee').addEventListener('click',()=>{const n=Number($('sTdee').value);if(!Number.isFinite(n)||n<1000||n>6000)return;const s=read('fuel-settings',{});write('fuel-settings',{...s,tdee:Math.round(n)});renderAll()})}$('sTdee').value=settings().tdee}
function renderAll(){installSettings();renderHome()}
