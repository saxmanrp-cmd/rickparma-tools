  ...
  properties: {
    category: {
      ...
      enum: [
        ...
        'not_interested','not_booking_contact','follow_up_later','submission_redirect','out_of_office','bounce','opt_out',
        'question','other'
      ]
    },
...
export async function draftBookingEmail(env, { prospect, artistContext, assets, purpose = 'initial', priorMessages = '' }) {
  const input = `Write a concise, human professional booking email from Rick Parma.\n\nPURPOSE: ${purpose}\nPROSPECT:\n${JSON.stringify(prospect)}\n\nARTIST FACTS:\n${String(artistContext || '').slice(0, 7000)}\n\nUSEFUL LINKS:\n${String(assets || '').slice(0, 4000)}\n\nPRIOR THREAD IF ANY:\n${String(priorMessages || '').slice(0, 8000)}\n\nRules: 1) Do not invent facts about the venue, contact, artist, availability, rates, or relationships. 2) Keep a first cold email compact and personalized; usually 120-220 words before the compliance footer. 3) Treat Rick's EPK as the primary promo/media link. For an initial cold email, usually include the EPK plus the current calendar only when useful; do not add separate YouTube promo links unless the recipient specifically asks for video or the EPK would be insufficient. 4) Do not claim Rick is represented by the recipient or imply an existing relationship unless supplied. 5) Do not promise availability, pricing, contracts, exclusivity, or dates. 6) The caller is Rick Parma, not a fake agent. 7) No hypey marketing language or mass-mail wording. 8) If relatedOpportunities are supplied for the same buyer email, treat them as one relationship and do not write separate-sounding room pitches. 9) If prospect.contactName is blank, use a generic greeting such as Hello; NEVER infer a person's name from an email address, URL, evidence summary, or other field. 10) BODY MUST BE PLAIN TEXT ONLY. Write URLs as raw URLs on their own lines; never use Markdown links, brackets, HTML, or embedded hyperlinks. 11) For an initial cold email, use no more than two total links unless the recipient specifically requests more. Prefer the EPK over the general website and over standalone social links. 12) Do not repeat the same website or EPK URL in both the body and signature.`;
