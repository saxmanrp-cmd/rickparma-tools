...
if (!tests.includes("test('Reach Intelligence is wired into Max Reach'")) {
  tests += [
    ...
    "  for (const needle of ['id=\"reachIntelligence\"','id=\"reachFitScore\"','id=\"useReachTimeBtn\"','id=\"useReachCaptionBtn\"','/reach-intelligence.js']) assert.equal(html.includes(needle), true, 'HTML missing ' + needle);",
    "  for (const needle of ['buildReachIntelligence','classifyIntent','nextSuggestedSlot','Learning mode','timingMode','Caption starter added']) assert.equal(reach.includes(needle), true, 'Reach Intelligence missing ' + needle);",
